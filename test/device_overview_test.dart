...
class StubAdbService extends AdbService {
  ...
  Future<AdbResult> run(List<String> args) async {
    if (shouldFail) {
      return const AdbResult(exitCode: 1, stdout: '', stderr: 'error: device offline');
    }
  ...
  @override
  Future<AdbResult> shellArgs(String deviceId, List<String> args) async {
    if (shouldFail) {
      return const AdbResult(exitCode: 1, stdout: '', stderr: 'error: device offline');
    }
    if (args.contains('getprop')) {
      return const AdbResult(
        exitCode: 0,
        stdout: '[ro.product.marketname]: [Redmi K40]\n'
...
void main() {
  ...
  group('DeviceInfoService Cache & Fallback', () {
    ...
    test('should save to cache on success and load from cache on failure', () async {
