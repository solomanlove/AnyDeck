...
2. **核对公共 UI 组件**
   ...
   - **表格与提示组件**：重复的 Table Header、排序图标、Snack/Toast、状态 Banner 必须优先抽到 `presentation/widgets/` 下的公共工具类或不可变 Widget，避免每个 Tab 各自维护一份样式。
   - **注释要求**：新增公共组件或工具类必须用一句中文注释说明职责；只解释边界、性能保护或生命周期原因，不给显而易见的属性赋值写注释。
